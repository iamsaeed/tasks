    ...
    <slide-over :open="open" @close="close" :title="!isEditable ? 'Add Task Status' : 'Edit Task Status'">
        ...
        <form class="space-y-3" @submit.prevent="addTaskStatus" method="post">
            <div>
                ...
                <div class="mt-1">
                    <input placeholder="Status Name" v-model="current_task_status.name" autofocus type="text" autocomplete="email" class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
...
export default {
    ...
    created() {
        if(this.isEditable){
            this.current_task_status = this.task_status;
        }
    },
    methods : {
        reset(){
        ...
        addTaskStatus(){
            let _this = this;
            axios.post('/add_task_status', _this.current_task_status).then(response => {
                _this.reset();
                _this.close();
            }).catch(error => {
